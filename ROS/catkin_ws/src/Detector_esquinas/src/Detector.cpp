...
void getContours(cv::Mat imgDil, cv::Mat img){
  ...
  if (contours.size()>0) //SI HAY ALGÚN CONTORNO
  {
    ...
    for (int i = 0; i < contours.size(); i++)
    {
      ...
      if(objCor==12) //SI TIENE 12 VERTICES
      {
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    cv::Scalar lower(0,0,0);       //(0,0,0)       (0,0,0)   
    cv::Scalar upper(179,207,223); //(0,0,45-55)   (179,207,223)  
    ...
    cv::rectangle(cv_ptr->image, cv::Point((0.27*ancho),(0.12*alto)),cv::Point((0.73*ancho),(0.88*alto)), cv::Scalar(255,0,0),2); //171,58  468,422
