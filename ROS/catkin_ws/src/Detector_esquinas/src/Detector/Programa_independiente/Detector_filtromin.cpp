...
void getContours(Mat imgDil, Mat img){
  ...
  //hierarchy = {{1, -1, -1, -1}, {2,  0, -1, -1},{3,  1, -1, -1},{4,  2, -1, -1},{5,  3, -1, -1}};
  ...
  for (int i = 0, area_min=contourArea(contours[0]); i < contours.size(); i++)
  {
    ...
    float peri = arcLength(contours[min], true);
    approxPolyDP(contours[min], conPoly[min], 0.02 * peri, true);
    boundRect[min] = boundingRect(conPoly[min]);
    //Dibujamos el contorno con cada vertice y luego dibujamos el rectángulo que encuadra
    rectangle(img, boundRect[min].tl(), boundRect[min].br(), Scalar(0, 255, 0), 2);
}
