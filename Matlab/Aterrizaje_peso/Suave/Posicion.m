...
bag = rosbag('todo.bag');
pos = select(bag,'Time',[bag.StartTime+1.5 bag.EndTime-2.36],'Topic','/drone1/self_localization/pose'); 
msgStructs = readMessages(pos,'DataFormat','struct');

%Se inicializa las variables
n=size( msgStructs,  1); %Numero mensajes
tiempo_total=bag.EndTime-bag.StartTime-4.66;
suma_x=0;  
suma_y=0;  
xi=2.6*(ones(n,1));   %x ideal
yi=1.75*ones(n,1);      %y ideal
zi=0.22*(ones(n,1)); %z ideal
x=zeros(n,1);     
y=zeros(n,1); 
z=zeros(n,1);
RMSE_x=zeros(n,1);
