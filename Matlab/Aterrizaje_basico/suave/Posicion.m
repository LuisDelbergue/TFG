...
bag = rosbag('todo.bag');
pos = select(bag,'Time',[bag.StartTime+11.5 bag.EndTime-17.95],'Topic','/drone1/self_localization/pose'); 
msgStructs = readMessages(pos,'DataFormat','struct');

%Se inicializa las variables
n=size( msgStructs,  1); %Numero mensajes
tiempo_total=bag.EndTime-bag.StartTime-29.45;
suma_x=0;  
suma_y=0;  
xi=2.43*(ones(n,1));   %x ideal
yi=-0.33*ones(n,1);      %y ideal
