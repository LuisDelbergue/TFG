...
bag = rosbag('todo.bag');
pos = select(bag,'Time',[bag.StartTime+2.5 bag.EndTime-4.5],'Topic','/drone1/self_localization/pose'); 
msgStructs = readMessages(pos,'DataFormat','struct');

%Se inicializa las variables
n=size( msgStructs,  1); %Numero mensajes
tiempo_total=bag.EndTime-bag.StartTime-7;
suma_x=0;  
suma_y=0;  
xi=2.48*(ones(n,1));   %x ideal
yi=-0.64*ones(n,1);      %y ideal
