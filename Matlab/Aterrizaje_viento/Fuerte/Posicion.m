...
bag = rosbag('todo.bag');
pos = select(bag,'Time',[bag.StartTime+8 bag.EndTime-2.5],'Topic','/drone1/self_localization/pose'); 
msgStructs = readMessages(pos,'DataFormat','struct');

%Se inicializa las variables
n=size( msgStructs,  1); %Numero mensajes
tiempo_total=bag.EndTime-bag.StartTime-10.5;
suma_x=0;  
suma_y=0;  
xi=2*(ones(n,1));   %x ideal
yi=0*ones(n,1);      %y ideal
zi=0.23*(ones(n,1)); %z ideal
x=zeros(n,1);     
y=zeros(n,1); 
z=zeros(n,1);
RMSE_x=zeros(n,1);
